...
void eliminarNodo(Nodo *&,int); // Indicamos la existencia de la función eliminarNodo()
void eliminarLista(Nodo *&,int &); // Indicamos la existencia de la función eliminarLista()
...
void menu(){
    ...
    do{
        ...
        switch(opcion){
            case 1: cout<<"Digite un número: "; cin>>dato; // Solicitamos un dato al usuario para insertarlo dentro de la lista
            ...
            case 2: mostrarLista(lista); // Mostramos todos los elementos de la lista
                    ...
                    system("pause");
                    break;
            case 3: cout<<"Digite un número: "; cin>>dato; // Solicitamos un dato al usuario para buscarlo dentro de la lista
                    buscarLista(lista,dato); // Buscamos en la lista el dato ingresado por el usuario
                    cout<<"\n";
                    system("pause");
                    break;
            case 4: cout<<"Digite el elemento a eliminar: "; cin>>dato; // Solicitamos un dato al usuario para eliminarlo de la lista
                    eliminarNodo(lista,dato); // Eliminamos de la lista el dato ingresado por el usuario
                    cout<<"\n";
                    system("pause");
                    break;
            case 5: while(lista != NULL){ // Repetimos el proceso de eliminar elementos de la lista hasta que quede vacía
                        eliminarLista(lista,dato);
                        cout<<dato<<" -> "; // Mostramos el elemento eliminado
                    }
                    cout<<"(Datos eliminados)"; // Indicamos que la lista quedó vacía
                    cout<<"\n";
                    system("pause");
                    break;    
        }
        system("cls"); // Limpiamos la pantalla para mostrar el menú
    }while(opcion != 6); // Repetimos el proceso hasta que el usuario elija la opción 'Salir'
}
...
void buscarLista(Nodo *lista, int n){ // Función que busca un elemento 'n' en la lista
    ...
    Nodo *actual = lista; // Puntero para recorrer la lista desde el inicio
...
void eliminarNodo(Nodo *&lista,int n){
    if(lista != NULL){ // Verificar que la lista no está vacía
        Nodo *aux_borrar; // Declaramos el puntero al nodo que se desea eliminar
        Nodo *anterior = NULL; // Declaramos el puntero al nodo anterior

        aux_borrar = lista; // Iniciamos desde el primer nodo

        while((aux_borrar != NULL) && (aux_borrar->dato != n)){ // Buscamos el nodo a eliminar recorriendo la lista
            anterior = aux_borrar; // Guardamos el nodo anterior
            aux_borrar = aux_borrar->siguiente; // Avanzamos al siguiente nodo
        }

        if(aux_borrar == NULL){ // El elemento NO ha sido encontrado en la lista
            cout<<"\tEl elemento "<<n<<" No ha sido encontrado";
        }
        
        else if(anterior == NULL){ // El primer elemento es el que vamos a eliminar
            int eliminado = aux_borrar->dato; // Guardamos el valor eliminado
            lista = lista->siguiente; // Actualizamos el inicio de la lista
            delete aux_borrar; // Liberamos la memoria del nodo eliminado
            cout<<"\tElemento "<<eliminado<<" ha sido eliminado de la lista";
        }
       
        else{ // El elemento está en la lista pero no es el primer nodo
            anterior->siguiente = aux_borrar->siguiente; // Saltamos el nodo a eliminar
            int eliminado = aux_borrar->dato; // Guardamos el valor eliminado
            delete aux_borrar; // Liberamos la memoria del nodo eliminado
            cout<<"\tElemento "<<eliminado<<" ha sido eliminado de la lista";
        }
...
void eliminarLista(Nodo *&lista,int &n){ // Función que deja la lista vacía
    Nodo *aux = lista; // Declaramos el puntero al primer nodo de la lista
    n = aux->dato; // Guardamos el valor del nodo eliminado
    lista = aux->siguiente; // Actualizamos el inicio de la lista
